    ...
    for j = 1:m
        for i = 1:n
            ...
            if i == 1 && j == 1
                ...
                push!(val, val_push)
                push!(b, bd_coff[3] / bd_coff[2] + S[1, 1])
                continue
            end
            if i == n && j == 1
                val_push = [
                ...
                push!(val, val_push)
                push!(b, bd_coff[3] / bd_coff[2] + S[n, 1])
                continue
            end
            if i == n && j == m
                val_push = [
            ...
            if i == 1
                ...
                push!(b, bd_coff[3] / bd_coff[2]*sqrt(gamma[i,j]) + S[i,j])
                continue
            end
            if i == n
            ...
            if j == 1
                ...
                push!(val, val_push[findall(x->x==true,to_val)])
                push!(b, bd_coff[3] / bd_coff[2]*sqrt(alpha[i,j]) + S[i,j])
                continue
            end
            if j == m
                val_push[6] = rho*V[i,j]/4-mu*0.25*(beta[i,j]/Ja[i,j]+beta[i,j-1]/Ja[i,j-1])-mu*0.25*(gamma[i,j]/Ja[i,j]+gamma[i-1,j]/Ja[i-1,j])
